...
Part of the model is not convergent and the coefficient of date is small enough. Thus the trend of Gang_Activity is also negligible.
...
m_Days_Gang <- brm(Days_Gang ~ EmployHrs_pm + EmployWg_pm + EmployHrs_pmc + EmployWg_pmc + (EmployHrs_pmc + EmployWg_pmc | ID), data = GangEpl, seed = 2152)
```
...
[JZ] # (Is it Ok that the model had 42 divergent transitions after warmup?)

Model Equations:

Lv-1:

$$\text{Days\_Gang}_{ij} = \beta_{0j} + \beta_{1j} \text{EmployHrs\_pmc}_{ij} + \beta_{2j} \text{EmployWg\_pmc}_{ij} + e{ij}$$
Lv-2:

$$
\begin{aligned}
  \beta_{0j} & = \gamma_{00} + \gamma_{01} \text{EmployHrs\_pm}_j + \gamma_{02} \text{EmployWg\_pm}_j + u_{0j}  \\
...
```{r Days_Gang_2, cache=TRUE, results='hide', eval=FALSE}
m_Days_Gang_2 <- brm(Days_Gang ~ EmployHrs_pm + EmployHrs_pmc + (EmployHrs_pmc | ID), data = GangEpl, seed = 2152)
```

```{r results_m_Days_Gang_2, eval=FALSE}
summary(m_Days_Gang_2)
```

<!-- This is a second try without predictors of EmoloyWg. The result demonstrated that both level1 and level2 variables of EmployHrs are not significantly predicting Days_Gang. -->

## Using variables to predict Gang_Activity

```{r Gang_Activity, cache=TRUE, results='hide'}
m_Gang_Activity <- brm(Gang_Activity ~ EmployHrs_pm + EmployWg_pm + EmployHrs_pmc + EmployWg_pmc + (EmployHrs_pmc + EmployWg_pmc | ID), data = GangEpl, seed = 2152)
```
...
A second try without EmployWg:

```{r Gang_Activity_2, cache=TRUE, results='hide'}
m_Gang_Activity_2 <- brm(Days_Gang ~ EmployHrs_pm + EmployHrs_pmc + (EmployHrs_pmc | ID), data = GangEpl, seed = 2152)
```

```{r results_m_Gang_Activity_2}
summary(m_Gang_Activity_2)
```

<!-- This is a second try without predictors of EmoloyWg. The result demonstrated that both level1 and level2 variables of EmployHrs are not significantly predicting Gang_Activity. -->

## Using variables to predict GangMem

```{r GangMem, cache=TRUE, results='hide'}
m_GangMem <- brm(GangMem ~ EmployHrs_pm + EmployWg_pm + EmployHrs_pmc + EmployWg_pmc + (EmployHrs_pmc + EmployWg_pmc | ID), data = GangEpl, seed = 2152)
```
...
```{r GangMem_2, cache=TRUE, results='hide', eval=FALSE}
m_GangMem_2 <- brm(Days_Gang ~ EmployHrs_pm + EmployHrs_pmc + (EmployHrs_pmc | ID), data = GangEpl, seed = 2152)
```

```{r results_m_GangMem_2, eval=FALSE}
summary(m_GangMem_2)
```

<!-- This is a second try without predictors of EmoloyWg. The result demonstrated that both level1 and level2 variables of EmployHrs are not significantly predicting GangMem. -->
